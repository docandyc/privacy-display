...
已有实验结果显示：`n=4, 240Hz` 下 FPI 约 0.030，CIEDE2000 色差约 0.37；Tesseract、EasyOCR、PaddleOCR 在 120 样本语料上的原始帧字符识别率分别为 94.0%±9.1%、94.1%±8.1%、94.9%±7.3%，单子帧识别率均约为 0.0%，配对降幅达到 93.9%–94.9%；单子帧词级准确率、exact-match、敏感 token 恢复率同样为 0.0%。但覆盖完整周期的时域平均、相位搜索和蓝通道 max 重构可高比例还原：120 样本强攻击逐样本择优后字符恢复率为 95.2%（95%CI 93.8%–96.5%）、exact-match 为 91.7%、敏感 token 恢复率为 98.6%，泄露率为 100%。这是线性时域方案的原理边界。项目已新增在线 VLM 可读性评测入口，可对同一批攻击帧调用 SiliconFlow / Qwen3-VL 并在本地计算恢复指标；本次 live 尝试受沙箱 DNS/用量限制影响，生成的是全 API-error 诊断文件，不能引用为 VLM 防御率。项目还新增 `publication_summary` 与 `reproducibility_manifest` 两类产物：前者统一论文表格来源并会把全失败 VLM 文件标记为不可引用，后者记录环境、命令、Git 状态和关键文件哈希，适合作为投稿补充材料的复现清单。
...
1. 早期论文稿中引用的 `Kaleido: You can watch it but cannot record it` 和 `LiShield: Automating visual privacy protection using a smart LED`，公开检索未找到可核验记录。当前已从论文正文与参考文献中移除，替换为可追溯的 ScreenAvoider、Eye-Shield、DeepLight、Revelio、BRIGHTNESS 与抗屏摄文档水印文献；后续投稿应继续避免使用无 DOI、无 arXiv、无 publisher landing page 的引用。
...
已按这一方向补强项目：`CameraSimulator` 新增相位搜索重构、加权差分累加和单通道恢复攻击，`experiments/attack_analysis.py` 会输出强相机攻击者结果并保存 `experiments/results/attack_analysis_strong_camera.json`，语料级结果保存为 `experiments/results/corpus_strong_camera_attack.json`。当前全量结果显示，完整周期平均、相位搜索与蓝通道 max 在 120 样本上均可恢复 94%–95% 字符，逐样本最强攻击字符恢复率 95.2%，而 luma/blue 差分累加仍为 0%。同时新增 `src/attack/vlm_evaluator.py`、`src/evaluation/vlm_benchmark.py` 和 `experiments/vlm_readability_analysis.py`：VLM 只接收图像和 JSON 转写指令，不接收 ground truth，结果本地计算字符、词级、exact-match 和敏感 token 恢复指标，避免评估污染；本次 `experiments/results/vlm_qwen3_siliconflow.json` 仅记录联网失败诊断，`publication_summary` 会标记为 all_calls_failed。真实设备侧新增 `experiments/real_capture_analysis.py` 与 `experiments/real_captures/metadata_template.json`，用于导入手机/智能眼镜拍摄图片并生成 `real_capture_ocr.{json,md}`；尚未导入真实拍摄素材，因此仍不能引用真实设备数值。剩余可继续补 rolling-shutter row alignment、时域 super-resolution、TrOCR、有效在线 VLM/手机系统 OCR 和真实拍摄数据采集。

### 6.2 强化真实设备评测

至少选 3 类相机：普通手机、带高帧率/专业模式手机、智能眼镜或头戴设备。每类记录曝光时间、帧率、分辨率、是否自动曝光、距离、角度和环境光。项目已提供 `experiments/real_capture_analysis.py --init-template` 生成 metadata 模板，采集后可统一导入分析。实验组合建议：
...
隐私指标不只用 OCR accuracy。当前已补充 WER/词级准确率、exact-match 与敏感字段恢复率，并加入在线 VLM 读屏攻击的可复现实验入口。默认命令 `python experiments/vlm_readability_analysis.py --dry-run --samples-per-category 1` 会抽取 9 个类别、8 类攻击帧，共 72 次模型调用；真实在线运行需要有效网络和 `SILICONFLOW_API_KEY`，本次沙箱内尝试只产生全 API-error 诊断文件，不能引用 VLM 数值。项目还新增 `python experiments/publication_summary.py`，把 OCR、强相机攻击、目标检测、离轴攻击和可选 VLM 结果汇总为 `experiments/results/publication_summary.{json,md}`，作为论文表格的单一核对入口；`python experiments/reproducibility_manifest.py` 会输出 `experiments/results/reproducibility_manifest.json`，记录环境、复现命令、Git 状态和关键文件哈希，且只记录在线 VLM 所需环境变量名、不记录密钥值。下一步还应完成有效 VLM 在线调用、扩展多轮 VLM 问答、手机系统 OCR、重构后 SSIM/LPIPS 和真实设备采集。安全结论以“文本恢复率”作为主指标，视觉指标作为可用性约束。
...
- `scripts/reproduce_all.sh` 已作为统一入口：默认执行测试、VLM dry-run、`publication_summary` 与 `reproducibility_manifest`，`--full-offline` 重跑较慢离线实验，`--with-vlm-live` 才执行真实在线 VLM；
- 每次更新论文表格前先运行 `publication_summary`，归档前再运行 `reproducibility_manifest`，或直接运行默认 `scripts/reproduce_all.sh` 使表格来源和文件完整性分离可审计；
...
1. 增加真实相机/手机拍摄实验，至少覆盖自动曝光、短曝光、视频流和不同角度。
2. 在可联网环境中完成有效在线 VLM OCR 结果，再加入 TrOCR、手机系统 OCR 与真实拍摄样本，避免仅覆盖桌面截图和开源 OCR。
3. 继续补强强攻击者：rolling-shutter row alignment、时域 super-resolution、更多 VLM 读屏模型与多轮问答。
4. 把完整周期平均和相位搜索/蓝通道 max 失守写成理论边界和负面结果贡献。
5. 用 OKLAB/蓝通道/空间自适应调制做一版感知优化，与当前 RGB 随机点阵做消融对比。
6. 准备可复现仓库和补充材料：用 `scripts/reproduce_all.sh` 刷新 `publication_summary` 与 `reproducibility_manifest`，补有效 VLM/手机真实拍摄数据后，再按 IEEE Access 模板重写为英文 14-18 页。
