...
## Typography Review
- Resolved fonts: `Times New Roman` (48), `Arial` (29)
- No obvious font availability or source-font mismatch items found.

## Module Checklist
### `physical_scene`
- Frame: invisible logical audit region
- Children (30): `panel_a_title`, `authorized_line1`, `authorized_line2`, `sensitive_line1`, `sensitive_line2`, `phone_line1`, `phone_line2`, `chair_back`, `chair_seat`, `chair_support`, `user_torso`, `user_head`, `user_hair`, `user_arm`, `desk_top`, `desk_cabinet`, `desk_leg`, `keyboard`, `bookshelf`, `bookshelf_lines`, `sensitive_display`, `display_screen`, `display_private`, `display_data`, `display_stand`, `display_base`, `phone_hand`, `smartphone`, `phone_screen`, `phone_lens`
...
### `subframe_region`
- Frame: invisible logical audit region
- Children (17): `subframe_1`, `subframe_grid_1`, `subframe_1_base`, `subframe_2`, `subframe_grid_2`, `subframe_2_base`, `subframe_3`, `subframe_grid_3`, `subframe_3_base`, `subframe_4`, `subframe_grid_4`, `subframe_4_base`, `selection_left`, `selection_right`, `subframe_label_1`, `subframe_label_2`, `subframe_label_3`
- Incoming edges (0): none
- Outgoing edges (5): `frame1_to_eye`, `frame2_to_eye`, `frame3_to_eye`, `frame4_to_eye`, `selected_to_camera`
- Internal edges (3): `sequence_1_to_2`, `sequence_2_to_3`, `sequence_3_to_4`
- [ ] Compare this module against the source crop: frame bounds, child count, labels, colors, and arrow directions.
- [ ] Check every outgoing edge: does it originate from a component, a boundary, or a bus in the source?

### `human_region`
- Frame: invisible logical audit region
- Children (12): `panel_b_title`, `human_eye_label`, `human_eye`, `human_pupil`, `integration_line1`, `integration_line2`, `readable_display`, `readable_screen`, `readable_glyph_grid`, `readable_stand`, `readable_base`, `readable_label`
...
### `camera_region`
- Frame: invisible logical audit region
- Children (12): `camera_icon`, `camera_lens`, `camera_top`, `camera_label`, `sampling_label_1`, `sampling_label_2`, `fragment_display`, `fragment_grid`, `fragment_base`, `fragment_label_1`, `fragment_label_2`, `ocr_failure`
- Incoming edges (1): `selected_to_camera`
- Outgoing edges (0): none
- Internal edges (4): `camera_to_fragment`, `fragment_to_ocr`, `ocr_x_down`, `ocr_x_up`
- [ ] Compare this module against the source crop: frame bounds, child count, labels, colors, and arrow directions.
- [ ] Check every outgoing edge: does it originate from a component, a boundary, or a bus in the source?

## Topology Review Items
- [ ] `bookshelf_lines` is a compact paper grid/vector without source_bbox_px; count/spacing mistakes are hard to catch without source-region binding.
- [ ] `readable_glyph_grid` is a compact paper grid/vector without source_bbox_px; count/spacing mistakes are hard to catch without source-region binding.
...
- `sequence_1_to_2`: `lane_arrow` `horizontal` `subframe_region` -> `subframe_region`
- `sequence_2_to_3`: `lane_arrow` `horizontal` `subframe_region` -> `subframe_region`
- `sequence_3_to_4`: `lane_arrow` `horizontal` `subframe_region` -> `subframe_region`
...
- `ocr_x_down`: `line_segment` `straight` `camera_region` -> `camera_region`
- `ocr_x_up`: `line_segment` `straight` `camera_region` -> `camera_region`
