...
%% ============================================================
\begin{abstract}
Screen content can be covertly photographed and transcribed by optical character recognition (OCR) or vision-language models (VLMs). We present temporal pixel masking, a display method exploiting differences between human temporal integration and short camera exposures. A base frame is decomposed into rapidly alternating, cryptographically randomized complementary subframes. We evaluate a 240-Hz software prototype using one USB Video Class (UVC) webcam. In the primary matched short-exposure analysis (12 content items $\times$ 8 common-setting geometries $\times$ 3 repeats), best-of-engine OCR character recovery decreased from 94.5\% unprotected to 17.8\% for the readability-priority profile and 5.6\% for the high-suppression profile; paired content-cluster contrasts were 76.7 and 88.9 percentage points, respectively. The latter meets the descriptive $<$10\% recovery target. In the unbalanced nine-geometry sensitivity pool, 150-frame temporal averaging recovered 47.9--71.1\% of characters. In an attacker-favorable, adequate-exposure 0.5\,m VLM session, the strongest of three tested VLMs reached 77.8\% exact match, with recovery concentrated on large-font short snippets. Under long exposure, masking could increase recovery over the unprotected baseline, consistent with reduced luminance shifting the sensor into its linear range. These results establish feasibility for mitigating conventional OCR in the tested short-exposure UVC-camera link, but not protection against temporal integration or VLM attackers. In 56 participants, readability-priority masking retained 94.83\% typing accuracy but reduced speed by 1.14 words per minute (WPM); readability, stability, and comfort averaged 3.79, 2.98, and 3.39 out of 5, indicating measurable usability costs.
\end{abstract}
...
We consider an attacker with a direct or oblique line of sight to a display showing static sensitive text. The attacker can photograph the screen from a nearby position and process the resulting image without accessing the host computer, application, network, or source document. Protected assets include credentials, financial data, personally identifiable information (PII), source code, and confidential documents. The authorized user is expected to read and interact with the protected display directly. The system does not prevent image acquisition; its narrower objective is to reduce machine-recoverable text in selected physical capture conditions while retaining usable visual content for the authorized user.

\subsection{Attacker Capabilities and Knowledge}

We organize the evaluated attacker along acquisition, temporal coverage, and recognition dimensions rather than as a strictly monotonic hierarchy. The \emph{target attacker} obtains a short-exposure snapshot and applies conventional per-image OCR; this tier defines the primary feasibility claim. An \emph{enhanced attacker} can vary viewing geometry, apply deterministic image preprocessing, select among recognition engines, or use a vision-language model (VLM). A \emph{boundary attacker} can extend the exposure window, record a sustained sequence, align and aggregate multiple frames, or perform phase-aware reconstruction. The exact operational settings used for these tiers are specified in \S\ref{sec:setup}, and the corresponding attack results are reported in \S\ref{sec:real_capture}--\S\ref{sec:security_frontier}.

We adopt a public-algorithm threat model: the attacker knows the temporal masking algorithm, subframe count~$n$, refresh rate, profile composition, and candidate period parameters. In the implementation, pixel-to-slot assignments and output permutations are generated with a ChaCha20-based cryptographically secure pseudorandom number generator (CSPRNG)~\cite{b_chacha2008}; a hash-based message authentication code using SHA-256 (HMAC-SHA256) derives a subkey for each cycle. The attacker is not assumed to receive the current key or exact starting phase before capture, but neither value is treated as a security anchor. Temporal averaging can accumulate information without either value, and an adaptive attacker can estimate alignment from a captured sequence. Once registered observations cover a complete complementary cycle, key secrecy cannot prevent reconstruction in the ideal linear model.
...
\subsection{Operating Profiles and Scope}
\label{sec:hardened}

Because the system is configurable, the evaluation uses named, composable operating profiles rather than treating one parameter setting as universally appropriate. Unless otherwise stated, the profiles use $n=4$ temporal frames and the complementary perturbation described above. Table~\ref{tab:profile_definitions} defines the components and intended evaluation role of each profile. The exact parameter instances used in the physical-capture evaluation are reported separately in Section~\ref{sec:setup}; this separation distinguishes a reusable system interface from the particular configurations evaluated in this paper.
...
Strong & \checkmark & \checkmark & Stripe and glyph overlays & Readability-oriented artifact condition \\
Readability-priority & \checkmark & \checkmark & Stripe/glyph overlays and partial inversion & Usability-facing composite condition \\
...
The nonlinear profile combinations are intentionally not interpreted as the sum of their isolated components. In particular, the high-suppression profile explores a more aggressive point on the recovery--readability trade-off and should not be read as a recommended default deployment setting. For reproducibility, the paper-facing \emph{high-suppression} profile corresponds to the archival implementation identifier \texttt{capture\_hardened} (legacy identifier \texttt{vlm}); these names distinguish the evaluated role rather than assert a universal security property.
...
\textbf{Evaluated configurations}: Table~\ref{tab:profile_composition} reports the exact profile instances used in the physical-capture evaluation. These settings are experimental configurations rather than universal implementation defaults. In particular, the readability-priority capture condition invokes the \texttt{strong} profile with explicit stripe and glyph opacity overrides of 0.10 and 0.12, respectively, and a partial inversion frame with $\alpha=0.20$; the reusable \texttt{strong} command-line default uses different overlay amplitudes. The high-suppression condition uses the canonical \texttt{capture\_hardened} profile (legacy tag \texttt{vlm}).

\begin{table*}[!t]
\caption{Exact Profile Instances Used in the Physical-Capture Evaluation ($n=4$ for All Profiles). The Readability-Priority Row Is the Archived \texttt{deployed} Condition and Explicitly Overrides the Reusable \texttt{strong} Defaults.}
...
\textbf{OCR aggregation and uncertainty}: Character and whole-text metrics are maximized independently across the three engines for each capture. Explicit fields use the union of fields recovered by the three engines. Duplicate captures with the same profile, content, position, and repeat are averaged before matching. Thus, the 288 units in each primary profile comprise 12 content items, 8 geometries, and 3 repeats; they are not treated as 288 independent population samples. The primary comparison uses keys present in all three profiles. Paired contrasts resample the 12 content items as clusters with 2,000 resamples and seed 20260612, holding the tested geometries fixed. These intervals quantify sensitivity across the archived content clusters.

\textbf{Common-setting analysis}: The d0.5/$15^{\circ}$ position used a different nominal UVC setting and produced visibly underexposed protected frames. The remaining 8 geometries define the primary fixed-setting result (Table~\ref{tab:real_ocr_common}). The unbalanced 9-geometry archive is retained only as sensitivity evidence in Table~\ref{tab:real_ocr_full_pool}; it does not replace the matched primary comparison.
...
\begin{table*}[!t]
\caption{Primary Matched Short-Exposure OCR Comparison at the Common Nominal 3.91-ms UVC Setting. Duplicate Captures Are Averaged Before Matching; $N=288$ per Profile Comprises 12 Content Items $\times$ 8 Geometries $\times$ 3 Repeats. Paired contrasts resample the 12 content items as clusters while holding the 8 tested geometries fixed; they do not treat the 288 units as independent population samples. The noncomparable d0.5/$15^{\circ}$ run is excluded.}
...
\begin{table*}[!t]
\caption{Single-UVC Real-Capture OCR Sensitivity Pool (Best-of-Engine; $N$ = Captures per Row; All 9 Geometries). This is an unbalanced all-available archive, not the primary comparison. Brackets are 95\% capture-resampling summaries, not population confidence intervals. Each video row averages 150 frames for one content--position video unit (nominally 2.5\,s at 60\,fps). The primary matched estimate is Table~\ref{tab:real_ocr_common}.}
...
\caption{Best-of-engine OCR character recovery and exact match for three representative real-capture profiles (unprotected, readability-priority, and high-suppression) under short exposure, long exposure, and a 150-frame video temporal mean, pooled across the 9 tested distance--angle configurations. This unbalanced sensitivity pool is distinct from the primary matched comparison in Table~\ref{tab:real_ocr_common}. Lower bars indicate less recovered text. The long-exposure and video bars expose the integration boundary: the readability-priority profile retains substantial recoverable content even when its short-exposure bar is low. Readability-priority sample counts include the disclosed five-item repeat and therefore differ from the other profiles.}
...
\begin{enumerate}
\item \textbf{VLMs recover substantially more text than conventional OCR}: In the selected attacker-favorable, adequate-exposure 0.5\,m session, best-of-engine OCR yields 8.4\% char / 0\% exact on high-suppression short exposure, while Qwen3-VL achieves 91.5\% char and exactly recovers 28/36 captures. The alternative underexposed session yields 0/36 Qwen exact matches, so the 0.5\,m value is a session-conditioned boundary observation rather than a stable estimate. At 1.5\,m, the three models retain 33.3--47.2\% exact / 61.5--70.0\% char. The gap between conventional OCR and modern VLMs is large.
...
\begin{table}[!t]
\caption{Real-Capture MOT17-09-FRCNN Still-Frame Tracking (ByteTrack-Style Greedy Association, 450 Frames; HOTA and IDF1 from the Archived Run-Recorded Tracking-Metric Backend; HOTA Uses TrackEval When Available~\cite{b_hota2021})}
\label{tab:real_mot}
\centering
\footnotesize
\begin{tabular}{llcc}
...
When an attacker accumulates registered frames covering a full cycle, complementary subframes can be re-summed in the ideal linear model of~(\ref{eq:noise_complement}), with the archived per-sample strongest digital attack recovering 95.2\% and pure full-cycle temporal averaging recovering 94.3\% (Table~\ref{tab:digital_boundary})---values that empirically mark the reconstruction boundary of the evaluated linear decomposition. For high-suppression, the 150-frame mean yields 47.9\% recovery in the unbalanced nine-geometry sensitivity pool and 53.9\% after excluding the underexposed d0.5/$15^{\circ}$ position. Character recovery also depends on content and the physical imaging path.
...
On the 8 COCO images, single-subframe mAP50 across the four detectors is 0--6.0\%. Across the 5,316 MOT frames, single-subframe IDF1 is 0.1--5.4\%, while MOTA includes negative and nonmonotonic values. Negative MOTA occurs when the false-positive, false-negative, and identity-error penalties exceed the number of ground-truth objects; it is not a negative detection probability. Detector architecture, confidence thresholds, proposal density, and the greedy associator jointly shape the observed differences. Fig.~\ref{fig:all_attackers} summarizes the residual capability of a single protected subframe relative to the clean baseline across the evaluated OCR, detection, and tracking pipelines.
...
The primary result is a matched short-exposure comparison in the tested S600 display--camera link. Across 12 content items, 8 common-setting geometries, and 3 repeats, best-of-engine character recovery falls from 94.5\% for unprotected content to 17.8\% for readability-priority and 5.6\% for high-suppression. The corresponding content-cluster contrasts are 76.7 and 88.9 percentage points. Although the physical archive contains captures from nine geometries, the d0.5/$15^{\circ}$ run used different nominal UVC controls and is excluded from this primary inference. Thus, the result establishes feasibility for mitigating conventional OCR under the evaluated short-exposure conditions, rather than a general guarantee against screen photography.
...
The physical temporal-averaging attack likewise recovers substantial text without the mask seed or phase. In the unbalanced nine-geometry sensitivity pool, the 150-frame mean reaches 71.1\% recovery for readability-priority and 47.9\% for high-suppression. Excluding the underexposed d0.5/$15^{\circ}$ run yields 79.7\% and 53.9\%, respectively; this difference illustrates sensitivity to exposure and geometry rather than a general performance gain. Sustained capture can therefore convert temporal fragments into a usable recognition signal, while the extent of recovery remains coupled to the evaluated imaging conditions.
...
The VLM probes show that conventional OCR is not an adequate proxy for all recognition attackers. On the protected photographs, recovery is strongest for large-font credentials, code, digits, and short sentences, while dense CET6 pages are more strongly suppressed. The 0.5\,m results, however, come from the selected attacker-favorable adequate-exposure session; the alternative underexposed session and nonrandom parseability failures preclude a stable cross-session or cross-endpoint ranking. The appropriate interpretation is therefore a session-conditioned attack boundary: high-suppression can reduce conventional OCR recovery while stronger recognition still recovers substantial large-font content. Future profiles should be evaluated against glyph scale, semantic predictability, and field sensitivity rather than assumed to provide VLM-robust protection.
...
The statistical and attack evidence is also bounded. Content-cluster intervals hold the tested geometries fixed, and the 12-item corpus is too small for script-level inference. The 150-frame experiment does not map the duration--recovery curve. Detection uses 150 COCO images at one position, while tracking uses 450 still-display-then-capture frames from one MOT17 sequence. The VLM probes use one on-axis profile, three API endpoints, and raw JPEG inputs; response failures and the post-hoc selection of the adequate-exposure 0.5\,m session further restrict the interpretation. Commercial OCR, learned restoration, and unconstrained adaptive preprocessing may recover additional text.
