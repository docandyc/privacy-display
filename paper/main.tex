...
\PARstart{S}{creens} fill offices, banks, hospitals, and conference rooms, and so do capture devices: smartphones, smart glasses~\cite{b_wang2026} (e.g., Ray-Ban Meta), and miniature cameras. A single photograph of a screen suffices to extract text via optical character recognition (OCR) or identify interface elements through object detection. This \emph{visual eavesdropping} is covert: it leaves no network traces and requires no physical contact with the target device. Controlled experiments and field studies confirm that such attacks are common, succeed at high rates, and frequently go unnoticed~\cite{b_ponemon2016,b_eiband2017}.
...
This study compares conventional OCR recovery across three display profiles at one UVC control setting. The instrumented pipeline stores one image per target for subsequent OCR. Across the 8 common-setting geometries, matched mean recovery was 94.5\% unprotected, 17.8\% for readability-priority, and 5.6\% for high-suppression. The high-suppression profile missed its descriptive $<5\%$ target.
...
\S\ref{sec:related} reviews prior work, \S\ref{sec:threat} defines the threat model, and \S\ref{sec:method} presents the design. \S\ref{sec:evaluation} reports OCR evidence, detection/tracking stress tests, VLM and integration boundaries, and the deferred usability protocol. \S\S\ref{sec:discussion} and~\ref{sec:conclusion} discuss the evidence hierarchy.
...
\item \textbf{Extended attacks}: Long exposure, 150-frame temporal averaging, off-axis capture, phase-aware channel reconstruction, object detection and association, and commercial vision-language models (\S\ref{sec:vlm}).
\end{itemize}
...
To test temporal masking against stronger recognizers, we applied three commercial VLM endpoints exposed through the SiliconFlow application programming interface (API): Qwen3-VL-32B-Instruct~\cite{b_qwen3vl2025}, Kimi-K2.6, and GLM-4.5V~\cite{b_glm45v2025}. The comparison reveals how model capability changes recovery from the same protected photographs.
...
\begin{enumerate}
\item \textbf{VLMs recover substantially more text than conventional OCR}: On the same 0.5\,m photographs, best-of-engine OCR yields 8.4\% char / 0\% exact on high-suppression short exposure, while Qwen3-VL achieves 91.5\% char and exactly recovers 28/36 captures. At 1.5\,m, the three models retain 33.3--47.2\% exact / 61.5--70.0\% char. These results expose a capability gap between conventional OCR and modern VLMs.
...
\item \textbf{Sustained video aggregation strongly increases VLM recovery}: At 0.5\,m, missingness bounds for the 150-frame mean span 58.3--91.7\% exact and 61.2--95.5\% character recovery across models. In the failure-free 1.5\,m session, the 150-frame mean yields 58.3--66.7\% exact and 88.5--90.3\% character recovery, while single-best-frame exact recovery is 0--8.3\%.
\end{enumerate}

Together, the VLM probes sharpen the paper's practical contribution. High-suppression strongly reduces dense small-text recovery under conventional OCR, while VLMs recover many large-font credentials, code snippets, and digit strings. This contrast identifies content-adaptive, VLM-aware temporal protection as the next technical target.
...
This layered evidence advances temporal display privacy in three ways. First, it adds a 10,575-capture physical measurement to prior temporal-display studies. Second, it connects average OCR recovery to explicit-field leakage and upper-tail risk. Third, it evaluates attacker escalation across preprocessing, sustained capture, and modern recognition models.
...
Physical temporal averaging also increases recovery without requiring the mask seed or phase. The 150-frame mean reaches 71.1\% for readability-priority and 47.9\% for high-suppression. At the common settings, these values rise to 79.7\% and 53.9\%. These results quantify how sustained capture converts temporal fragments into a stronger recognition signal.

\subsection{Profile Trade-offs and Practical Implications}

The two principal profiles occupy distinct operating points. Readability-priority preserves a digitally reconstructed SSIM of 0.948 but retains 17.7\% explicit-field recovery. High-suppression lowers several recovery measures while reducing digital SSIM to 0.764 and increasing $\Delta E_{00}$ from 1.46 to 4.87. This profile architecture makes the privacy--readability trade-off explicit and tunable.

The high-suppression inversion slot also reaches 93.0\% recovery after contrast stretching. This result identifies slot-level composition as a critical design variable. A practical system can therefore combine temporal masking with field-level policies and independent access controls for high-value content.

\subsection{VLM-Aware Implications}
\label{sec:vlm_defense}

Modern VLMs recover far more text than conventional OCR from the same protected photographs. Recovery is strongest for large-font credentials, code, digits, and short sentences. Dense CET6 pages remain more strongly suppressed. This content dependence reveals a concrete next-generation design target: temporal profiles should respond to glyph scale, semantic predictability, and field sensitivity.
...
Detection and tracking stress tests show the same directional change across four detectors and provide a secondary cross-task map. Overall, the study offers a reproducible measurement framework connecting profile strength, OCR capability, and temporal integration. It provides a quantitative system-design basis for comparing future temporal display-privacy methods and attacks.
