...
\title{Temporal Pixel Masking and Conventional OCR Recovery in Short-Exposure Screen Capture: A Single-Camera UVC Study}
...
%% ============================================================
\begin{abstract}
Screen content can be silently captured and processed by optical character recognition (OCR) or vision-language models (VLMs). This study measures conventional-OCR recovery from temporally masked screens on one USB Video Class (UVC) camera link. The pipeline partitions each frame into rapidly alternating complementary subframes and adds profile-dependent overlays. We collected 10,575 physical captures across 9 distance--angle configurations. The primary common-setting analysis uses the same 288 duplicate-averaged short-exposure units (12 content items $\times$ 8 geometries $\times$ 3 repeats) for each of three profiles. Per-capture best-of-engine mean character recovery was 94.5\% unprotected, 17.8\% for readability-priority, and 5.6\% for high-suppression. Readability-priority retained 17.7\% field-micro exact recovery across 432 annotated field opportunities, with 95.0\% matched P99 character recovery. A fixed five-transform, three-engine oracle raised readability-priority recovery to 40.2\% and high-suppression to 13.7\%. A 150-frame mean and on-axis VLM probes recovered substantially more text. The study provides a 10,575-capture evaluation on a single UVC link and maps recovery under fixed preprocessing, temporal averaging, and VLM recognition.
\end{abstract}
...
This study compares conventional OCR recovery across three display profiles at one UVC control setting. The instrumented pipeline archives each capture as an image for offline recognition. Across the 8 common-setting geometries, matched mean recovery was 94.5\% unprotected, 17.8\% for readability-priority, and 5.6\% for high-suppression. The high-suppression profile missed its descriptive $<5\%$ target.
...
The paper-facing name ``high-suppression'' reflects the profile's relative recovery in Table~\ref{tab:profile_composition}. The archived identifier remains \texttt{capture\_hardened} (with legacy tag \texttt{vlm}) for reproducibility; \S\ref{sec:real_capture} reports the target miss and residual leakage.
...
Fig.~\ref{fig:multiengine_ocr} visualizes these results. Single-subframe recovery does not exceed 2.6\% for any engine, confirming a consistent pattern across all three recognizers.
...
The software, experimental configurations, de-identified per-sample OCR results, VLM records, and analysis scripts are maintained at \url{https://github.com/docandyc/privacy-display}. The repository is currently mutable and is not presented as an immutable submission artifact. A frozen release tag or archival DOI will be provided with the camera-ready version to support reproducibility. VLM records include endpoint identifiers, timestamps, prompts, decoding parameters, input hashes or paths, parse status, and raw outputs. The archived identifiers \texttt{capture\_hardened} and \texttt{vlm} both denote high-suppression. If the user study is conducted, identifiable registration data will not be released; only de-identified results and audits will be public.
