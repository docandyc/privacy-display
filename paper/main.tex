...
\PARstart{S}{creens} fill offices, banks, hospitals, and conference rooms, and so do capture devices: smartphones, smart glasses~\cite{b_wang2026} (e.g., Ray-Ban Meta), and miniature cameras. A single photograph of a screen suffices to extract text via optical character recognition (OCR) or identify interface elements through object detection. This \emph{visual eavesdropping} is covert: it leaves no network traces and requires no physical contact with the target device. Controlled experiments and survey studies confirm that such attacks are common, succeed at high rates, and frequently go unnoticed~\cite{b_ponemon2016,b_eiband2017}.

\subsection{Existing Approaches and Their Limitations}

Current screen-privacy solutions each address a narrow threat subset. Spatial, software, watermarking, and temporal approaches cover different viewing geometries or attack stages, but none reports the same combination of on-axis short-exposure capture, static sensitive text, and post-capture machine recognition (\S\ref{sec:related}). That gap motivates measuring machine recovery from physical snapshots.
...
Our approach exploits the gap between human temporal integration and camera exposure windows. Each frame is decomposed into $n$ randomly complementary subframes that alternate at high speed. When the exposure is shorter than or close to one subframe duration, the camera records only an incomplete pixel subset, whereas the human observer integrates content across subframes (Fig.~\ref{fig:concept}). Long exposure or video reconstruction covering a full cycle can re-sum the complementary subframes.

This study compares conventional OCR recovery across three display profiles at one UVC control setting. The instrumented pipeline archives each capture as an image for offline recognition. Across the 8 common-setting geometries, matched mean recovery is 94.5\% unprotected, 17.8\% for readability-priority, and 5.6\% for high-suppression. The high-suppression profile misses its descriptive $<5\%$ target.

Attack-oriented evaluation expands this result. A temporal-mean attack averaged 150 short-exposure frames, nominally 2.5\,s at 60\,fps, and vision-language model (VLM) probes recovered substantial large-font snippets (\S\ref{sec:vlm}). These tests show where recovery rises beyond the primary conventional-OCR setting.
...
\S\ref{sec:related} reviews prior work, \S\ref{sec:threat} defines the threat model, and \S\ref{sec:method} presents the design. \S\ref{sec:evaluation} reports OCR evidence, VLM probes, detection/tracking stress tests, simulation diagnostics, and the deferred usability protocol. \S\S\ref{sec:discussion} and~\ref{sec:conclusion} discuss the evidence hierarchy.
...
Each pixel is illuminated only once per $n$-slot cycle. A factor-$n$ light-output increase during the active slot would restore the original cycle-averaged luminance in the linear-light model. The current proof of concept (PoC) uses fixed panel output. CIEDE2000 $\Delta E_{00}$~\cite{b_ciede2000} and SSIM~\cite{b_wang2004} quantify digitally integrated and normalized reconstructions across configurations.

\subsection{Timing and Bandwidth}

For a basic cycle containing only $n$ complementary subframes, taking 60\,Hz as the minimum cycle rate requires $f_r\geq60n$; at $n=4$ this corresponds to 240\,Hz. If the cycle also includes one inversion frame, the requirement becomes $f_r\geq60(n+1)$, and at 240\,Hz the actual cycle rate drops to only 48\,Hz. Thus, 240\,Hz meets the basic four-subframe configuration but not the inversion-frame configuration's 60\,Hz cycle target. Human flicker perception also depends on luminance, contrast, retinal eccentricity, spatial content, and eye movements~\cite{b_cai2024,b_davis2015}; nominal cycle rate alone cannot predict comfort.
...
The current PoC operates at the application layer, using Python and GPU rendering to generate display sequences. This architecture provides direct control over subframe composition, playback order, and profile parameters during evaluation.
...
The camera was placed at 0.5\,m, 1\,m, and 1.5\,m from the display, rotated to $0^{\circ}$, $15^{\circ}$, and $30^{\circ}$ at each distance. The manufacturer specifies an 8-megapixel 4K sensor and 1080p/60\,fps output~\cite{b_emeet_s600}. This grid provides nine distance--angle configurations for the physical evaluation.
...
High-suppression & 288 & 5.6\% & 88.9\,pp [85.5, 91.9] & Exploratory stress profile; misses descriptive $<5\%$ target \\
\bottomrule
\end{tabular}
\end{table*}
...
High-suppression strongly reduces dense small-text recovery under conventional OCR, but VLMs still recover many large-font credentials, code snippets, and digit strings. Content-adaptive, VLM-aware temporal protection is the logical next step.
...
When an attacker accumulates registered frames covering a full cycle, complementary subframes can be re-summed in the ideal linear model of~(\ref{eq:noise_complement}). The archived per-sample strongest digital attack recovers 95.2\%, while pure full-cycle temporal averaging recovers 94.3\%. These values empirically mark the reconstruction boundary of the evaluated linear decomposition. The high-suppression profile yields 47.9\% pooled recovery under the 150-frame mean and 53.9\% after excluding the underexposed position. Character recovery also depends on content and the physical imaging path.
...
Relative to prior temporal-display studies, the 10,575-capture physical archive is new. So is the link from average OCR recovery to explicit-field leakage and upper-tail risk, and the evaluation of attacker escalation across preprocessing, sustained capture, and modern recognition models.

\subsection{Integration and Attacker Escalation}

Temporal masking exploits the difference between human temporal integration and short camera exposure. In the ideal linear model, registered complementary subframes can reconstruct the original because $\sum\mathbf{I}_k = \mathbf{I}$. The digital attack evaluation confirms this boundary, reaching 95.2\% per-sample strongest recovery and 94.3\% under pure full-cycle averaging.

Physical temporal averaging also increases recovery without requiring the mask seed or phase. The 150-frame mean reaches 71.1\% for readability-priority and 47.9\% for high-suppression. At the common settings, these values rise to 79.7\% and 53.9\%. Sustained capture thus converts temporal fragments into a usable recognition signal.

\subsection{Profile Trade-offs and Practical Implications}

The two principal profiles occupy distinct operating points. Readability-priority preserves a digitally reconstructed SSIM of 0.948 but retains 17.7\% explicit-field recovery. High-suppression lowers several recovery measures while reducing digital SSIM to 0.764 and increasing $\Delta E_{00}$ from 1.46 to 4.87. The privacy--readability trade-off between these profiles is explicit and tunable.

The high-suppression inversion slot also reaches 93.0\% recovery after contrast stretching, making slot-level composition a weak point in the current design. A practical system should therefore combine temporal masking with field-level policies and independent access controls for high-value content.

\subsection{VLM-Aware Implications}
\label{sec:vlm_defense}

Modern VLMs recover far more text than conventional OCR from the same protected photographs. Recovery is strongest for large-font credentials, code, digits, and short sentences; dense CET6 pages remain more strongly suppressed. The implication is that temporal profiles should adapt to glyph scale, semantic predictability, and field sensitivity.
...
The planned user study has not yet collected participant data. It evaluates readability-priority rather than high-suppression, and its immediate-comfort rating does not address long-term or clinical effects. Driver-level deployment would also require framebuffer integration, panel coordination, and verified timing. Temporal masking should therefore complement established access controls when protecting credentials and other high-value fields.

Future work should randomize profile order, lock camera parameters, record batch time, and measure physical slot timing and luminance. Matched static controls can separate temporal and luminance effects. Replication across cameras and displays can test transfer, while continuous-video attacks can map recovery against recording duration. VLM-aware profiles can explore font-size-adaptive granularity, glyph decoys, and field-level rendering. Completing the preregistered usability study would connect machine-recovery gains to human readability and comfort.
...
Attack-oriented evaluation recovers substantially more information. A 150-frame temporal mean reaches 71.1\% character recovery for readability-priority and 47.9\% for high-suppression; the common-setting values are 79.7\% and 53.9\%. In the full 9-geometry sensitivity pool, long-exposure recovery is 60.9\% for readability-priority versus 47.3\% unprotected. On-axis VLM probes also recover substantial large-font content, while the three-engine preprocessing oracle reaches 40.2\% for readability-priority and 13.7\% for high-suppression.

Detection and tracking stress tests show the same directional change across four detectors. The archived data, scripts, and per-sample records are public and can serve as a baseline for comparing future temporal display-privacy methods and attacks.
