...
Strong-overlay (anti-OCR) & \checkmark & \checkmark & Stripe and glyph overlays & Isolate the effects of stripe and glyph overlays \\
...
\begin{table*}[!t]
\caption{Primary Matched Short-Exposure OCR Comparison at the Common Nominal 3.91-ms UVC Setting. Duplicate Captures Are Averaged Before Matching. $N=324$ per Profile Comprises 12 Content Items $\times$ 9 Geometries $\times$ 3 Repeats. Paired contrasts resample the 12 content items as clusters while holding the 9 tested geometries fixed. The 324 units are not treated as independent population samples. All contrasts are computed from unrounded per-unit values and may therefore differ by up to 0.1\,pp from the difference of the rounded means shown in the table.}
...
\item \textbf{Fixed preprocessing defines a stronger OCR attack}: Across the same 324 matched units per profile, the raw three-engine oracle yields 94.1\% unprotected, 16.2\% readability-priority, and 5.0\% high-suppression recovery. Allowing the five fixed transforms raises these values to 95.5\%, 37.2\%, and 13.2\%, respectively (Table~\ref{tab:preprocessing_grid}). The paired unprotected-minus-readability contrast is 58.4 percentage points ([53.4, 62.8]), while the unprotected-minus-high-suppression contrast is 82.4 points ([78.1, 85.9]); as in Table~\ref{tab:real_ocr_common}, both contrasts are computed from unrounded per-unit values rather than from the rounded means quoted above. The complete 19,926-cell matrix has no OCR errors.

\item \textbf{Profiles show distinct component and attacker-dependent target behavior}: In the duplicate-averaged summary, mask-only reaches 19.2\%, mask+noise reaches 25.6\%, and Strong-overlay reaches 20.7\%, revealing nonmonotonic component effects. Across the nine-geometry matched pool, readability-priority retains 15.7\% field-micro exact recovery (Table~\ref{tab:explicit_fields}). Its P50, P75, P90, P95, and P99 character-recovery values are 9.2\%, 18.7\%, 43.5\%, 61.4\%, and 94.8\%, respectively. The corresponding high-suppression values are 1.6\%, 8.3\%, 14.1\%, 18.7\%, and 22.2\%. Under the primary raw-input, best-of-three OCR evaluation, high-suppression yields 5.0\% character recovery and 0.0\% exact match. It therefore satisfies its descriptive $<10\%$ character-recovery and $=0\%$ exact-match targets for that attacker. Under the fixed preprocessing grid, its character recovery rises to 13.2\%, exceeding the $<10\%$ character target, although exact match remains 0.0\%. High-suppression thus does not meet both targets against this enhanced short-exposure attacker. Readability-priority follows the same pattern. Under the primary raw-input evaluation, its relative reduction computed from the rounded means is $(94.1-16.2)/94.1=82.8\%$ and its duplicate-averaged exact-match rate is 0.5\%, satisfying both targets. Under fixed preprocessing, the analogous rounded-mean reduction falls to $(95.5-37.2)/95.5=61.0\%$ and exact match rises to 2.6\%, missing both the $\geq80\%$ and $<1\%$ targets.

\item \textbf{Integration attacks recover substantial text} (Fig.~\ref{fig:bar_attack}): Across the nine geometries, the readability-priority profile reaches 60.9\% char / 36.4\% exact under nominal long exposure and 73.4\% char / 43.5\% exact under the duplicate-averaged 150-frame mean. The latter aggregates 150 frames for each content--position video unit, corresponding nominally to 2.5\,s at 60\,fps. Weak inversion ($\alpha=0.2$) produces little integration reduction, and mask-only and Strong-overlay are similar under the 150-frame mean (79.0\% versus 78.6\%). High-suppression shows lower integration recovery.
...
\textbf{Per-engine results}: Table~\ref{tab:real_ocr_engine} breaks down duplicate-averaged raw-input short-exposure results by engine. The engines differ widely in unprotected recovery (Surya 37.1\%, EasyOCR 79.5\%, Tesseract 84.3\%). Under Strong-overlay, recovery ranges from 2.1--16.3\%; under readability-priority, 3.2--12.6\%; under high-suppression, 1.8--2.0\%. Table~\ref{tab:preprocessing_grid} and Fig.~\ref{fig:real_engine_ocr} use the full nine-geometry matched pool and show that every engine benefits from at least one fixed transform. For readability-priority, recovery rises from 3.2\% to 10.4\% for Tesseract, 12.6\% to 31.0\% for EasyOCR, and 3.6\% to 18.3\% for Surya. For high-suppression, the corresponding changes are 1.9\% to 7.6\%, 2.0\% to 5.8\%, and 1.8\% to 6.1\%.
...
We classified API outcomes as parseable transcriptions, explicit refusals or valid empty transcriptions, and unresolved technical failures. Explicit refusals and valid empty transcriptions are observed outcomes and receive zero exact match and zero character recovery. Provider, transport, and formatting failures were retried up to three times. Unresolved calls remain missing rather than being treated as evidence that an image was unreadable. Each cell therefore reports valid/planned calls and missing-response bounds over all planned inputs. The lower bound assigns zero recovery to every unresolved call, while the upper bound assigns 100\% recovery. These bounds are not confidence intervals. Conditional successful-call estimates are not used for model ranking, and cross-model ranges are restricted to cells with complete returns.

Of 1,188 planned requests, 1,133 (95.4\%) yielded parseable outputs, and 55 remained unresolved after three attempts. At 0.5\,m, failures comprised two Qwen non-JSON responses, 31 GLM HTTP 429 errors, and one GLM non-JSON response. At 1.0\,m, failures comprised four Qwen non-JSON responses, one Kimi connection closure, and 16 GLM TLS errors. No failures occurred at 1.5\,m. The archived errors therefore indicate provider, transport, or formatting failures rather than semantic evidence of unreadability. Their endpoint- and session-clustering also prevents an assumption of random missingness.
...
\toprule
Distance & Condition & OCR BoE & Qwen3-VL & Kimi-K2.6 & GLM-4.5V \\
\midrule
\multicolumn{6}{l}{\textit{Short-exposure distance sweep}}\\
\midrule
0.5\,m & original short & 58.3/92.4 & 83.3/94.5 [36/36] & 83.3/95.6 [36/36] & 52.8--80.6/66.4--94.2 [26/36] \\
0.5\,m & short & 0.0/8.4 & 77.8/91.5 [36/36] & 47.2/84.7 [36/36] & 44.4--58.3/61.0--74.9 [31/36] \\
1.0\,m & short & 0.0/3.6 & 55.6/79.1 [36/36] & 38.9/68.6 [36/36] & 25.0--33.3/43.1--51.4 [33/36] \\
1.5\,m & short & 0.0/6.5 & 47.2/68.9 [36/36] & 33.3/61.5 [36/36] & 36.1/70.0 [36/36] \\
\midrule
\multicolumn{6}{l}{\textit{Key attack views}}\\
\midrule
0.5\,m & long & 0.0/2.1 & 77.8--83.3/80.9--86.5 [34/36] & 2.8/7.9 [36/36] & 0.0--16.7/4.1--20.8 [30/36] \\
0.5\,m & Video (150-frame mean) & 8.3/50.7 & 83.3/94.0 [12/12] & 83.3/95.5 [12/12] & 58.3--91.7/61.2--94.5 [8/12] \\
1.5\,m & long & 0.0/13.2 & 91.7/89.3 [36/36] & 88.9/89.9 [36/36] & 58.3/62.4 [36/36] \\
1.5\,m & Video (single best) & 0.0/4.3 & 8.3/24.8 [12/12] & 0.0/12.7 [12/12] & 0.0/29.8 [12/12] \\
1.5\,m & Video (150-frame mean) & 0.0/48.4 & 66.7/90.3 [12/12] & 66.7/90.3 [12/12] & 58.3/88.5 [12/12] \\
1.5\,m & Video (window-mean best) & 0.0/8.6 & 58.3/89.0 [12/12] & 41.7/84.2 [12/12] & 66.7/87.2 [12/12] \\
1.5\,m & Video (max projection) & 0.0/19.0 & 66.7/90.1 [12/12] & 58.3/89.2 [12/12] & 58.3/88.7 [12/12] \\
\bottomrule
\end{tabular*}
}
\end{table*}
...
\textbf{Results}: Table~\ref{tab:study_typing} and Fig.~\ref{fig:study_typing} summarize the typing task. Relative to the control condition, the readability-priority full configuration changed mean accuracy by $-1.71$ percentage points (95\% CI $[-3.39, -0.04]$, Wilcoxon $W=428$, $p=0.043$, $r_{rb}=-0.33$). WPM changed by $-1.14$ (95\% CI $[-1.91, -0.39]$, $t(55)=-2.93$, $p=0.005$, $d_z=-0.39$). CPM changed by $-5.71$ (95\% CI $[-9.56, -1.88]$, $t(55)=-2.93$, $p=0.005$, $d_z=-0.39$). Attempted characters changed by $-1.74$ (95\% CI $[-2.88, -0.62]$, $t(55)=-2.98$, $p=0.004$, $d_z=-0.40$). First-keystroke latency changed by $+91.0$\,ms (95\% CI $[-31.1, +210.7]$, Wilcoxon $W=641$, $p=0.200$, $r_{rb}=0.20$). Under Holm correction across the five typing metrics, WPM, CPM, and attempted characters remain significant whereas the accuracy difference ($p=0.043$, rank 4 of 5) does not. Table~\ref{tab:study_ratings} and Fig.~\ref{fig:study_ratings} summarize the subjective ratings. Friedman tests yielded $\chi^2(5)=72.2$ ($p=3.5\times10^{-14}$) for readability, $\chi^2(5)=123.2$ ($p=6.5\times10^{-25}$) for stability, and $\chi^2(5)=98.5$ ($p=1.1\times10^{-19}$) for immediate visual comfort. For deployed full minus the unmasked anchor, readability gave Wilcoxon $W=58$, Holm-adjusted $p=2.6\times10^{-4}$, and $r_{rb}=-0.82$. Stability gave $W=16.5$, Holm-adjusted $p=2.2\times10^{-7}$, and $r_{rb}=-0.97$. Immediate visual comfort gave $W=21.5$, Holm-adjusted $p=1.8\times10^{-6}$, and $r_{rb}=-0.95$.
...
\begin{tabular}{lccc}
\toprule
OCR engine & Original frame & Single subframe & Reduction (pp) \\
\midrule
