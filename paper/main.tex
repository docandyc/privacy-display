...
The nonlinear profiles depart from strict completeness ($\sum\mathbf{I}_k \neq \mathbf{I}$), producing visible stripes and grain. In the static-content capture threat addressed here, completeness is the entry point for integration attacks. The high-suppression profile is designed to trade visible artifacts for reduced full-cycle recovery; \S\ref{sec:real_capture} reports the measured outcome.

The paper-facing name ``high-suppression'' reflects the profile's relative recovery in Table~\ref{tab:real_ocr_common}. The archived identifier remains \texttt{capture\_hardened} (with legacy tag \texttt{vlm}) for reproducibility; \S\ref{sec:real_capture} reports residual leakage and behavior under the preprocessing oracle.

\subsection{Partial Inversion Frame}

A long-exposure attack occurs when the exposure window covers a full display cycle ($n=4$ at 240\,Hz yields a basic cycle of approximately 16.7\,ms; with an inversion frame the $n+1$ slot cycle is approximately 20.8\,ms), where the archived long-exposure labels of 31.25 and 125\,ms are nominal values derived from UVC controls rather than photometric measurements. To perturb, rather than eliminate, full-cycle accumulation, the evaluated offline-playback and web pipelines append a \textbf{partial inversion frame} $\alpha(\mathbf{1}-\mathbf{I})$ (normalized domain $\mathbf{I}\in[0,1]$; 8-bit form $\alpha(255-\mathbf{I}_{8})$) after each $n$-subframe cycle, implemented as amplitude scaling over one additional display slot because vsync playback holds slot duration fixed. Higher $\alpha$ changes both the integrated signal and the digital reconstruction's luminance and contrast.
...
This trade-off runs opposite to Kaleido-class temporal re-encoding~\cite{b_kaleido2015}: Kaleido relies on perceptually complete mixing within a cycle to preserve viewing experience, so full-cycle integration reconstructs an approximation of the original, whereas the composite profiles here give up completeness because, for static content, completeness is the entry point for integration attacks.
