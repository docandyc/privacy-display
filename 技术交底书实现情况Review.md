...
| G2 | 检测模型从未实测 | 3.3 / 6.2 声称"YOLOv8 文本检测 mAP 0.92→0.08" | 代码中只有 yolov8/faster_rcnn 的代理噪声参数，无任何检测评测；该声称值无实验支撑（答辩风险点） | P1 |
...
| YOLOv8 文本检测 mAP 0.92→0.08 | **从未实测**（G2 补齐中） | 补实验后改用实测值，避免引用无支撑数字 |
...
- G1 本轮已完成，后续继续补 G2–G13 时应另开/沿用对应 Trellis 任务并分阶段提交
