...
- Analysis reports assignment-bucket balance and excludes minimum-view straight-line rating sessions as satisficing. The `flicker` DB field is presented as stability/no-flicker: higher is better, with 5 anchored as no perceptible flicker. The `fatigue` DB field is presented as immediate visual comfort: higher is better. The `privacy` DB field is presented as subjective anti-peeking effectiveness (`防偷窥效果`): higher means bystanders are perceived as less able to read the display.
