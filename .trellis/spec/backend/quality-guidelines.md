...
- `ratings` contains exactly six unique rows: `control_anchor`, `n2_mask_noise`, `n3_mask_noise`, `n4_mask_noise`, `n4_mask_only`, and `deployed_full`. Each has a unique order 0-5, a view duration of at least 10 seconds in non-debug sessions, timestamps, three 1-5 ratings, and `mask_meta`.
...
- Analysis reports assignment-bucket balance and excludes minimum-view straight-line rating sessions as satisficing using the three collected dimensions. The `flicker` DB field is presented as stability/no-flicker: higher is better, with 5 anchored as no perceptible flicker. The `fatigue` DB field is presented as immediate visual comfort: higher is better. The legacy nullable `privacy` column is retained only for migration compatibility; new submissions write `NULL`, and admin/export/analysis surfaces omit it.
