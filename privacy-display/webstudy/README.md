...
- `ratings`: six rows per completed participant, three 1--5 ratings, 10-second view duration/timestamps, and display timing metadata. The nullable `privacy` column is retained only to migrate old databases and remains `NULL` for new sessions.
...
- “稳定感” is coded so higher means less perceived flicker, with 5 anchored as no perceptible flicker. “即时视觉舒适感” is a single item after short exposure, not a clinical fatigue measure.
